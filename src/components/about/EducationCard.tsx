      ...
      <div data-testid="education-card-body" className="bg-background/80 px-4 py-4">
        ...
        {hasMetadata && (
          <div className="mt-3 flex flex-wrap items-center gap-2">
            {displayLocation && (
              <span className="border border-border bg-muted px-2.5 py-0.5 font-terminal text-sm text-foreground">
                {displayLocation}
              </span>
            )}
            {graduationDate && (
            ...
            {gpa && (
              <span className="ml-auto border border-border bg-muted px-2.5 py-0.5 font-terminal text-sm text-foreground">
                GPA: {gpa.split("/")[0]}
              </span>
            )}
          </div>
